...
const Admins = () => {
    ...
    const actions = (rowData) => {
        return (
            <>
                <Toast ref={toast} />
                <ConfirmDialog />
                <Button icon="pi pi-eye" severity="success" className="mr-1 w-auto h-auto" tooltip="View Order" tooltipOptions={{ position: 'top' }} text onClick={() => router.push(`/pages/add-order/${rowData.id}`)} />
    ...
    const emptyMessage = () => {
        return <h5 className='text-center pt-1' style={{ fontSize: '1em' }}>No Order to Display</h5>
    };
    const footerTemplate = {
        ...
        CurrentPageReport: (options) => {
            const name = (options.totalRecords > 1) ? 'Orders' : 'Order'
            return (
